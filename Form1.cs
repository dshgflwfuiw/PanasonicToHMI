...
namespace 松下变量转HMI
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            this.Text = "松下变量转HMI"; // 设置标题栏名称
            this.Icon = new System.Drawing.Icon(System.IO.Path.Combine(Application.StartupPath, "myicon.ico")); // 设置窗口图标，请确保myicon.ico文件在项目目录中
            ...
            // processor.ProcessWithDialog(this, Log);
        }
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    Log($"打开的文件路径: {inputPath}");
                    Log($"保存的文件路径: {outputPath}");

                    var processor = new CsvProcessor();
                    processor.Process(inputPath, Log);
                }
            }
        }
